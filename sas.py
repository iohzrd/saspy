...
class sas(object):
    ...
    def send_10000_bills_in_meters(self):
        ...
        if data != "":
            ...
            meters["s10000_bills_in_meter"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def send_20000_bills_in_meters(self):
        ...
        if data != "":
            ...
            meters["s20000_bills_in_meter"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def send_25000_bills_in_meters(self):
        ...
        if data != "":
            ...
            meters["s25000_bills_in_meter"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def send_50000_bills_in_meters(self):
        ...
        if data != "":
            ...
            meters["s50000_bills_in_meter"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def send_100000_bills_in_meters(self):
        ...
        if data != "":
            ...
            meters["s100000_bills_in_meter"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def credit_amount_of_all_bills_accepted(self):
        ...
        if data != "":
            ...
            meters["credit_amount_of_all_bills_accepted"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def coin_amount_accepted_from_external_coin_acceptor(self):
        ...
        if data != None:
            ...
            meters["coin_amount_accepted_from_external_coin_acceptor"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
    ...
    def number_of_bills_currently_in_stacker(self):
        ...
        data = self.__send_command(cmd, True, crc_need=False)
        if data != None:
            meters["number_bills_in_stacker"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
            return data
        return ""
    ...
    def total_credit_amount_of_all_bills_in_stacker(self):
        ...
        data = self.__send_command(cmd, True, crc_need=False)
        if data != None:
            meters["credits_SAS_in_stacker"] = int(
                binascii.hexlify(bytearray(data[1:5]))
            )
            return data
        return ""
    ...
    def set_secure_enhanced_validation_ID(
        self, MachineID=[0x01, 0x01, 0x01], seq_num=[0x00, 0x00, 0x01]
    ):
        # 4C
        cmd = [0x4C]
    ...
    def total_number_of_games_impimented(self):
        ...
        data = self.__send_command(cmd, True, crc_need=False)
        if data != None:
            meters["total_number_of_games_impemented"] = str(
    ...
    def game_meters(self, n=1):
        ...
        if data != None:
            ...
            meters["game_n_coin_out_meter"] = str(
                binascii.hexlify(bytearray(data[7:11]))
            )
            meters["game_n_jackpot_meter"] = str(
                binascii.hexlify(bytearray(data[11:15]))
            )
            meters["geme_n_games_played_meter"] = str(
    ...
    def AFT_register_gaming_machine(
        self, reg_code=0xFF, asset_number=0, reg_key=0, POS_ID=b"\x00\x00\x00\x00"
    ):
        # 73
        cmd = [0x73, 0x00]
        if reg_code == 0xFF:
    ...
    def AFT_game_lock_and_status_request(
        self, lock_code=0x00, transfer_condition=0b00000000, lock_timeout=0
    ):
        # 74
        cmd = [0x74]
    ...
    while True:
        ...
        if state == "67":  # cashin
            ...
            sas.redeem_ticket(
                ...
                parsing_code=0,
                validation_data=1234567891234567,
